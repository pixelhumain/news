...
var newsObj = {
	init : function (params) {
		...
		if(params.scroll && params.scroll!=="false"){
			...
			$(params.clickEvent).click(function(){
				loadStream(params);
			});
		}
		loadingData = false;
...
function getNewsObject(formId, actionType){
	...
	if(actionType=="save"){		
		...
		if( typeof custom != "undefined" && notNull(custom) && typeof custom.request != "undefined" && typeof custom.request.sourceKey != "undefined"){
			newNews.source = { 
				insertOrign : "custom",
				keys : [ 
					custom.request.sourceKey[0]
				],
				key : custom.request.sourceKey[0]
			};
		}
	}
